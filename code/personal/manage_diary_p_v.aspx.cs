...
public partial class manage_diary_p_v : System.Web.UI.Page
{
    public DataRow ds_p_v;

    public DataTable ds_p_v_pCom;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        ds_p_v = ShowPage().Rows[0];
        ds_p_v_pCom = ShowComm();
        this.Repeater1.DataSource = ds_p_v_pCom;
    ...
    {
        string sql = "select * from commentlist where d_id='" + id + "' order by c_time desc";
        return SqlHelper.ExecuteDataSetsql(sql).Tables[0];
    }
    ...
    //编辑
    protected void LinkButton_cedit(object sender, CommandEventArgs e)
    {
    ...
    protected void LinkButton_csec(object sender, CommandEventArgs e)
    {
        ...
        string sql = "update diarylisting set d_p_or_s='s' where d_id = " + id;
        if (SqlHelper.ExecuteNonQuery(sql) > 0)
        {
