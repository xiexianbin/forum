...
public partial class kynews : System.Web.UI.Page
{
    ...
    {
        AspNetPager1.RecordCount = SqlHelper.GetDataCount("select * from news where n_type='kynews'");
        SQL = "select * from news where n_type='kynews' order by n_time desc";

        if (!IsPostBack)
        {
            AspNetPager1.CurrentPageIndex = 1;
            ds_news_ky = SqlHelper.ExecuteDataTable(SQL, AspNetPager1);
        }
