<%@ Page Language="C#" AutoEventWireup="true" CodeFile="wr_those_years.aspx.cs" Inherits="news_wr_those_years" %>
...
<body>
    ...
    <div class="main">
        <div class="lfmain">
            ...
            <dl class="rjdl">
                <dt><span>考研推荐</span></dt>
                <%for (int i = 0; i < ds_ky_recommend.Rows.Count; i++)
                  { %>
                <dd class="lhotdiary">
                    <li class="title">
                        <%=ds_ky_recommend.Rows[i]["c_title"].ToString()%></a></li>
                </dd>
                <%} %>
            </dl>
        </div>
        <div class="rfmain">
            <dl class="rjdl">
                <dt><span>风雨那些年</span></dt>
            </dl>
            <dl>
                <%for (int i = 0; i < ds_wr_years.Rows.Count; i++)
                  { %>
                <dd class="NewsPages">
                    <a href="wr_years.aspx?id=<%=ds_wr_years.Rows[i]["n_id"].ToString() %>">
                        <li class="NewsPages_title">
                            <%=ds_wr_years.Rows[i]["n_title"].ToString()%></li></a>
                    <li class="NewsPages_time">
                        <%=ds_wr_years.Rows[i]["n_time"].ToString()%></li></dd>
                <% }%>
                <webdiyer:AspNetPager ID="AspNetPager2" runat="server" PageSize="20" AlwaysShow="true"
