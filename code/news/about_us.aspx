<%@ Page Language="C#" AutoEventWireup="true" CodeFile="about_us.aspx.cs" Inherits="news_about_us" %>
...
<body>
    ...
    <div class="main">
        <div class="lfmain">
            <dl class="rjdl">
                <dt><span>热门日记</span><a href="../diary/newdiary.aspx">更多>></a></dt>
                <%for (int i = 0; i < c_hot_diary; i++)
                  {
                      int id = int.Parse(ds_hot_diary.Rows[i]["d_id"].ToString()); %>
        ...
        <div class="rfmain">
            <dl class="rjdl">
                <dd class="news_s_title">
                    <%=ds_news["n_title"].ToString()%></dd>
                <dd class="news_s_time">
                    发表单位：<%=ds_news["ad_user"].ToString()%>&nbsp;&nbsp; 发表时间：<%=ds_news["n_time"].ToString()%>
                    &nbsp;&nbsp; 浏览量：(<%=ds_news["n_num"].ToString()%>)</dd>
                <dd class="news_s_text">
                    <%=ds_news["n_text"].ToString()%>
                </dd>
            </dl>
        </div>
    </div>
